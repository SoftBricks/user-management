...
UM.prototype.ROUTE_DEFAULT = {
    showUsers:      { name: "umShowUsers",      path: "/showUsers", waitOn: function() { return Meteor.subscribe('users');}},
    showUser:  		{ name: "umShowUser",  		path: "/showUser/:userId", waitOn: function() { return Meteor.subscribe('users');}},
    editUser:  		{ name: "umEditUser",  		path: "/editUser/:userId", waitOn: function() { return Meteor.subscribe('users');}},
    addUser:  		{ name: "umAddUser",  		path: "/addUser", waitOn: function() { return Meteor.subscribe('users');}},
    addGroup:  		{ name: "umAddGroup",  		path: "/addGroup", waitOn: function() { return [Meteor.subscribe('users'), Meteor.subscribe('groups')];}}
};
...
Meteor.startup(function() {
    var col;
    if(typeof usersWeSearch === "undefined"){
        col = Meteor.users;
    }else{
        col = usersWeSearch;
    }
